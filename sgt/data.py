...
class multi_heat_capacity(Dataset):
    def __init__(self,config,path,scaler
        ,is_train = True,
        **kwargs) -> None:
        self.is_train = is_train
        ...
        self.target = config['target']
        self.log = config['log']
        self.descriptors = pd.read_csv(config['descriptors'],index_col='jid')
    ...
    def __getitem__(self, idx):
        ...
        jid = self.df['id'][idx]
        # print(jid)
        features = self.descriptors.loc[jid][-273:].to_numpy()
        ...
        mask_id = torch.Tensor(mask_id)
        # print(mask_id)
        return cls_spg_wkf_token_id,composition_embeddings,mask_id,target,features
