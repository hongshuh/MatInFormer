...
if __name__ == '__main__':
    ...
    folder = os.path.join(config['output_folder'],f'seed_{seed_num}')
    print(folder)
            ...
            with torch.no_grad():
                ...
                for tokens_id,com_embed,mask_id,target,descriptors in val_loader:
                    ...
                    if config['task'] == 'classification':
                        ...
                        if config['log'] == True:
                            loss_val_all += nn.L1Loss(reduction='sum')(target.squeeze(),outputs.squeeze().exp())
                        else:
                ...
                if config['task'] == 'classification':
                    ...
                    checkpoint_path = f'{folder}/checkpoint.pth'
                    ...
                    if loss_val_all < best_validation_score:
                        ...
                        patience = 0
                        # torch.save(model)if current_score <= best_score:
                        best_path = f'{folder}/best_{seed}_{target_name}_{prompt}.pth'
                        torch.save(model.state_dict(),best_path)
                    ...
                    checkpoint_path = f'{folder}/checkpoint_{seed}_{target_name}_{prompt}.pth'
                    torch.save(model.state_dict(),checkpoint_path)
    ...
    with torch.no_grad():
        ...
        for tokens_id,com_embed,mask_id,target,descriptors in test_loader:
            ...
            if config['task'] == 'classification':
                ...
                target = torch.from_numpy(scaler.inverse_transform(target.cpu().reshape(-1, 1)))
                if config['log']:
                    pred_list += outputs.squeeze().detach().cpu().exp().numpy().tolist()
                else:
    ...
    df.to_csv(f'{folder}/test_results_{seed}_{target_name}_{prompt}.csv')
