...
class Matinformer_CGCNN(torch.nn.Module):
    def __init__(self,config):
        super().__init__()
        hidden_size = config['hidden_size'] 
        num_attention_heads = config['num_attention_heads']                 
...
class deMatInFormer(torch.nn.Module):
    def __init__(self,config):
        super().__init__()
        hidden_size = config['hidden_size'] 
        num_attention_heads = config['num_attention_heads']                 
        ...
        # self.Regressor = nn.Sequential(
        #     nn.Dropout(0.2),
        #     nn.Linear(hidden_size+273, 2*(hidden_size)),
        #     nn.SiLU(),
    ...
    def forward(self,tokens_id,com_embed,mask_id,descriptors):
        # tokens_id = torch.tensor(tokens_id,dtype=int)
        tokens_embed = self.word_embedding(tokens_id.type(torch.int))
        compo_embed = self.composition_word_embedding(com_embed)
        # print(tokens_embed.shape)
        ...
        # descriptors = self.de_embedding(descriptors)
        # descriptors = self.ln(descriptors)
        # # print(descriptors.shape)
        # # exit()
        # de_cls = torch.cat([logits,descriptors],axis=1)
        # output = self.Regressor(de_cls)
